...
//            listener.onExhibitionChange(change: .update, exhibitionPlants: fetchExhibitionPlants(), exhibition: Exhibition)
    ...
    func fetchOneExhibitionByName(exhibitionName:String) -> Exhibition?{
            let fetchRequest:NSFetchRequest<Exhibition> = Exhibition.fetchRequest()
            singleExhibitionController = NSFetchedResultsController<Exhibition>(fetchRequest:
            fetchRequest, managedObjectContext: persistentContainer.viewContext,
                          sectionNameKeyPath: nil, cacheName: nil)
            let predicate = NSPredicate(format: "ANY exhibitionName == %@", exhibitionName)
            fetchRequest.predicate = predicate
        ...
        if singleExhibitionController != nil{
            exhibition = (allExhibitionsFetchedResultsController?.fetchedObjects)![0]
        }
    ...
    func fetchOnePlantByName(scientificName:String) -> Plant?{
        let fetchRequest:NSFetchRequest<Plant> = Plant.fetchRequest()
        singlePlantController = NSFetchedResultsController<Plant>(fetchRequest:
        fetchRequest, managedObjectContext: persistentContainer.viewContext,
                      sectionNameKeyPath: nil, cacheName: nil)
        let predicate = NSPredicate(format: "ANY scientificName == %@", scientificName)
        fetchRequest.predicate = predicate
